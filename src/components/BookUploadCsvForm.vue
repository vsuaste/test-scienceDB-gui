<template>
  <div class="col-xs-5 content">
    <ul v-for="record in errors" v-if="errors" class="list-group">
      <li class="list-group-item">
        <div class="alert alert-danger">
          <h4>Errors for book {{record.record}}</h4>
          <ul>
    ...
    <h4>Upload books</h4>
      <form id="book-form" enctype="multipart/form-data" novalidate v-on:submit.prevent="onSubmit">
  ...
  methods: {
    onSubmit() {
      ...
      if (t.$refs.uploadTable.value.indexOf('.xlsx') > 0) {
        ...
        query = `mutation {bulkAddBookXlsx{ id }}`
      } else {
        var formElm = "csv_file"
        query = `mutation {bulkAddBookCsv{ id}}`
      }
